	...
	function init() {
		...
		for ( var i = 0; i < forms.length; i++ ) {
			...
			form.addEventListener(
				...
				function () {
					...
					var ak_bcc = prepare_array_for_request(
					...
					if ( this.getElementsByClassName ) {
						...
						var possible_akismet_containers =
						...
						for (
							...
							var container =
								possible_akismet_containers.item(
									containerIndex
								);

							if ( container.getAttribute( 'data-prefix' ) ) {
								akismet_field_prefix =
									container.getAttribute( 'data-prefix' );
								break;
							}
						}
					}
					...
					for ( var field_name in input_fields ) {
						...
						field.setAttribute(
							'name',
							akismet_field_prefix + field_name
						);
						field.setAttribute(
							'value',
				...
				function ( e ) {
					...
					if (
						...
						if ( -1 !== modifierKeyCodes.indexOf( e.key ) ) {
							...
							-1 !== correctionKeyCodes.indexOf( e.key )
						) {
		...
		document.addEventListener(
			...
			function ( e ) {
				...
				mouseclickCoordinate.push(
					rect.width > 0
						? Math.round( ( relativeX / rect.width ) * 100 )
						: 0
				);
				mouseclickCoordinate.push(
					rect.height > 0
						? Math.round( ( relativeY / rect.height ) * 100 )
						: 0
				);
