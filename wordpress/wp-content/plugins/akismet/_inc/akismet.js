jQuery( function ( $ ) {
	...
	function akismet_check_for_spam( offset, limit ) {
		...
		var percentage_complete =
			Math.round(
				( recheck_count /
					check_for_spam_buttons.data( 'pending-comment-count' ) ) *
					1000
			) / 10;
			...
			function ( result ) {
				if ( 'error' in result ) {
					...
					window.location.href =
						check_for_spam_buttons.data( 'failure-url' );
					return;
				}
				...
				if ( result.counts.processed < limit ) {
					...
					akismet_check_for_spam(
						offset + limit - result.counts.spam,
						limit
					);
				}
			}
	...
	if ( typeof MutationObserver !== 'undefined' ) {
		...
		var comment_list_container =
			document.getElementById( 'the-comment-list' );
	...
	function akismet_enable_comment_author_url_removal() {
		$( '#the-comment-list' )
			...
			.each( function () {
				if (
					$( this ).parent().find( '.akismet_remove_url' ).length > 0
				) {
					return;
				}
				...
				if ( linkHref.indexOf( currentHost ) != 0 ) {
					...
					$( this )
						.attr(
							'id',
							'author_comment_url_' + thisCommentId[ 1 ]
						)
						.after(
							$( '<a href="#" class="akismet_remove_url">x</a>' )
								...
								.attr(
									'title',
									WPAkismet.strings[ 'Remove this URL' ]
								)
						);
				}
			} );
